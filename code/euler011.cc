  ...
  int greatest_product = 0;
   ...
   for (int i = 0; i < m.size(); i++) {
      for (int j = 0; j < m[i].size(); j++) {
        //right.
        if ((j + 4) < m[i].size()){
          greatest_product = max(greatest_product, m[i][j]*m[i][j+1]*m[i][j+2]*m[i][j+3]);
        }

        //down
        if((i+4) < m.size()) {
          greatest_product = max(greatest_product, m[i][j]*m[i+1][j]*m[i+2][j]*m[i+3][j]);
        }

        //up diagonally
        if((j+4) < m[i].size() && (i-4) > 0){
          greatest_product = max(greatest_product, 
            m[i][j] * m[i-1][j+1] * m[i-2][j+2] * m[i-3][j+3]);
        }
        //down diagonally
        if((j+4) < m[i].size() && (i+4) < m.size()) {
          greatest_product = max(greatest_product,
            m[i][j] * m[i+1][j+1] * m[i+2][j+2] * m[i+3][j+3]);
        }
      }   
